...
function calcPassengers(feature) {
	...
	for (var i in clicked_layers) {
		...
		sum += data[is_home][cur_id][feature.properties['ID']-1][source];
	}
...
function loadData(id) {
	...
	$.ajax({url: '/cgi-bin/server.py',
			typle: 'get',
			data: {'request': 'DATA', 'clicked_id': id, 'map_id': 2, 'is_home': is_home},
			dataType: 'text',
...
function createLegendControl() {
    ...
    legend.onAdd = function(map) {
        ...
        this.legend.innerHTML = '<div class="row">' +
									'<button class="button" id=1 onClick="buttonClick(this.id)">פלאפון</button>' +
									'<button class="button" id=0 onClick="buttonClick(this.id)">GPS</button>' +
								'</div><div class="row">' +
									'<button class="button" id=3 onClick="buttonClick(this.id)">סלולרי</button>' +
									'<button class="button" id=2 onClick="buttonClick(this.id)">סלקום</button>' +
...
function buttonClick(id) {
	...
	if (id == "expand") {
		...
		document.getElementById(source).style.backgroundColor = '#ccc';
